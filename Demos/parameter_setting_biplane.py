...
def psf_pair_parameters():
    ...
    H, W = 81, 81  # in sensor [pixels]

    # safety margin from the boundary to prevent PSF truncation
    clear_dist = 20  # in sensor [pixels]

    # training z-range anf focus
    zmin = 0.5  # minimal z in [um] (including the axial shift)
    zmax = 3.5  # maximal z in [um] (including the axial shift)
    NFP = 2.0  # nominal focal plane in [um] (including the axial shift)
    ...
    blur_std_range = [0.65, 0.85]  # in sensor [pixels]
    ...
    training_data_path = path_curr_dir + "/TrainingImages_biplane/"
    ...
    results_path = path_curr_dir + "/Results_biplane/"
