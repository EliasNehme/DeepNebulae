# this script encapsulates all needed parameters for training/learning phase masks
...
def twomasks_parameters():
    ...
    # a transform set to identity means no change in  coordinates
    ...
    # registration error between the two cameras applied to PSF 2 channel
    reg_err = 0.0  # in [um]
    ...
    Hmask, Wmask = 229, 229  # in SLM [pixels]
    ...
    data_dims_dict = {'Hmask': Hmask, 'Wmask': Wmask, 'H': H, 'W': W, 'clear_dist': clear_dist, 
                      'zmin': zmin, 'zmax': zmax, 'NFP': NFP, 'D': D}
    ...
    nsig_ratio_range = [0.99, 1.01]  # in [counts]
    ...
    training_data_path = path_curr_dir + "/Training_learned_twomasks_zero_init/"
    ...
    results_path = path_curr_dir + "/Results_learned_twomasks_zero_init/"

    # maximal dilation flag when learning a localization CNN (set to None if learn_mask=True as we use a different CNN)
    dilation_flag = False  # if set to True then dmax=16 otherwise dmax=4
