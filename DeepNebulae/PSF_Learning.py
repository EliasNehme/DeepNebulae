...
from DeepNebulae.data_utils import PhasesOnlineDataset
from DeepNebulae.cnn_utils import OpticsDesignCNN
from DeepNebulae.loss_utils import KDE_loss3D, jaccard_coeff
from DeepNebulae.helper_utils import save_checkpoint, resume_from_checkpoint
from DeepNebulae.vis_utils import ShowLossJaccardAtEndOfEpoch, ShowMaskPSF


def learn_masks(setup_params):
    ...
    if (setup_params['learn_mask'][0] is False and setup_params['mask_init'][0] is None) or \
        (setup_params['learn_mask'][1] is False and setup_params['mask_init'][1] is None):
        raise ValueError('Non-learned masks should be initialized!')
    
    # zero-model init for non-initialized learned masks
    mask_zero = np.zeros((setup_params['Hmask'], setup_params['Wmask']))
    mask_init1 = setup_params['mask_init'][0] if setup_params['mask_init'][0] is not None else mask_zero
    mask_init2 = setup_params['mask_init'][1] if setup_params['mask_init'][1] is not None else mask_zero
    mask_param1 = nn.Parameter(torch.from_numpy(mask_init1).type(torch.FloatTensor).to(device))
    mask_param2 = nn.Parameter(torch.from_numpy(mask_init2).type(torch.FloatTensor).to(device))
    
    # learnable phase mask parameters
    mask_params = []
    if setup_params['learn_mask'][0]:    
        mask_params.append(mask_param1)
    if setup_params['learn_mask'][1]:
        mask_params.append(mask_param2)
    ...
    # adam optimizer
    optimizer = Adam(list(cnn.parameters()) + mask_params, lr=initial_learning_rate)
    ...
    if setup_params['resume_training']:
        ...
        mask_param1 = torch.load(path_save + 'mask1_best_loss.pkl')
        mask_param2 = torch.load(path_save + 'mask2_best_loss.pkl')
    ...
    for epoch in np.arange(start_epoch, end_epoch):
        ...
        with torch.set_grad_enabled(True):
            for batch_ind, (phases_emitter, Nphotons, targets) in enumerate(training_generator):
                ...
                optimizer.zero_grad()
                outputs = cnn([mask_param1, mask_param2], phases_emitter, Nphotons)
        ...
        if mean_valid_loss < (valid_loss_prev - 1e-4):
            ...
            torch.save(mask_param1.data, path_save + 'mask1_best_loss.pkl')
            torch.save(mask_param2.data, path_save + 'mask2_best_loss.pkl')
        ...
        if mean_valid_jacc > (valid_JI_prev + 1e-4):
            ...
            torch.save(mask_param1.data, path_save + 'mask1_best_jaccard.pkl')
            torch.save(mask_param2.data, path_save + 'mask2_best_jaccard.pkl')
