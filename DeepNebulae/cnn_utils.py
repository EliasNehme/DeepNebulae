...
from DeepNebulae.physics_utils import PhysicalLayer
...
class OpticsDesignCNN(nn.Module):
    def __init__(self, setup_params):
        ...
        self.layer1 = Conv2DLeakyReLUBN(2, 64, 3, 1, 1, 0.2)
        self.layer2 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer3 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer4 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer5 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer6 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer7 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer8 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer9 = Conv2DLeakyReLUBN(64 + 2, 64, 3, 1, 1, 0.2)
        self.layer10 = nn.Conv2d(64, setup_params['D'], kernel_size=1, dilation=1)
        self.pred = nn.Hardtanh(min_val=0.0, max_val=setup_params['scaling_factor'])

    def forward(self, masks, phase_emitter, Nphotons):
        
        # generate input images given current masks
        im1 = self.physicalLayer(masks[0], phase_emitter, Nphotons)
        im2 = self.physicalLayer(masks[1], phase_emitter, Nphotons)
