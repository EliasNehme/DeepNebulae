...
from DeepNebulae.physics_utils import Croplayer, Normalize01, MaskPhasesToPSFs, BlurLayer, NoiseLayer
...
class PhysicalLayerVisualization(nn.Module):
    ...
    def forward(self, mask, phase_emitter, nphotons):
        ...
        result_noisy = self.noise(images4D_crop_blur) if self.noise_flag else images4D_crop_blur

        # [0,1] normalization to prevent scaling volnurability
        result_noisy_01 = self.norm01(result_noisy) if self.norm_flag else result_noisy
...
def ShowRecNetInput(input_var, title_str):
    ...
    # extract phase mask from the model and convert it to numpy
    net_input = np.squeeze(input_var.data.cpu().numpy()) if is_tensor(input_var) else np.squeeze(input_var)
